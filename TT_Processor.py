...
class TTProcessor:
    def __init__(self, file_path, tt_name):
    ...
    def parse_accounting_file(self):
        ...
        while i < len(self.lines):
            ...
            if category_match:
                ...
                # 读取分类下的项目
                while i < len(self.lines) and not self.lines[i].startswith('###') and not self.lines[i].startswith('## Total'):
    ...
    def update_total_in_file(self):
        ...
        for i, line in enumerate(self.lines):
            if line.strip().startswith('Total :'):
                self.lines[i] = f'Total : {self.overall_total}\n'
                total_updated = True
        ...
        if not total_updated:
            self.lines.append('\n## Total\n```\n')
            self.lines.append(f'Total : {self.overall_total}\n')
            self.lines.append('```\n')
    ...
    def run(self):
        ...
        print(f'{self.tt_name} 总金额已更新为：{self.overall_total}')
